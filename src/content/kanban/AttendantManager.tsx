  ...
  useState(() => {
    const loadConfig = () => {
      if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
        ...
        if (localAtts) {
          try { setAttendants(JSON.parse(localAtts)); } catch (e) { console.warn(e); }
        }
        const localSettings = localStorage.getItem('settings');
        if (localSettings) {
          try { setSettings({ ...DEFAULT_SETTINGS, ...JSON.parse(localSettings) }); } catch (e) { console.warn(e); }
        }
      }
    };
    loadConfig();
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1, paddingRight: '4px', scrollbarWidth: 'thin' }}>
        ...
        <div style={{ background: 'rgba(15, 23, 42, 0.4)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: '#e2e8f0', cursor: 'pointer' }}>
            <input type="checkbox" checked={settings.quickAccess} onChange={(e) => handleSettingChange('quickAccess', e.target.checked)} style={{ accentColor: '#06b6d4' }} />
            Acesso rápido + atalho
          </label>

          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: '#e2e8f0', cursor: 'pointer' }}>
            <input type="checkbox" checked={settings.capitalizeInitial} onChange={(e) => handleSettingChange('capitalizeInitial', e.target.checked)} style={{ accentColor: '#06b6d4' }} />
            Letra inicial maiúscula
          </label>

          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: '#e2e8f0', cursor: 'pointer' }}>
            <input type="checkbox" checked={settings.transferAlert} onChange={(e) => handleSettingChange('transferAlert', e.target.checked)} style={{ accentColor: '#06b6d4' }} />
            Receber alerta de Transferência
          </label>

          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: '#e2e8f0', cursor: 'pointer' }}>
            <input type="checkbox" checked={settings.dontRepeatInChat} onChange={(e) => handleSettingChange('dontRepeatInChat', e.target.checked)} style={{ accentColor: '#06b6d4' }} />
            Não repetir no chat
          </label>

          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: '#e2e8f0', cursor: 'pointer' }}>
            <input type="checkbox" checked={settings.attendanceControl} onChange={(e) => handleSettingChange('attendanceControl', e.target.checked)} style={{ accentColor: '#06b6d4' }} />
        ...
        {isEditing ? (
          <form onSubmit={handleSave} className="quick-add-form" style={{ background: 'rgba(15, 23, 42, 0.4)', marginTop: 0 }}>
            ...
            <input type="text" placeholder="Mailson..." value={name} onChange={(e) => setName(e.target.value)} maxLength={25} required style={{ marginBottom: '12px' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#cbd5e1', cursor: 'pointer' }}>
                <input type="checkbox" checked={quebraLinha} onChange={(e) => setQuebraLinha(e.target.checked)} style={{ accentColor: '#06b6d4' }} />
                Quebra linha
              </label>

              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#cbd5e1', cursor: 'pointer' }}>
                <input type="checkbox" checked={negrito} onChange={(e) => setNegrito(e.target.checked)} style={{ accentColor: '#06b6d4' }} />
                Negrito
              </label>

              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#cbd5e1', cursor: 'pointer' }}>
                <input type="checkbox" checked={italico} onChange={(e) => setItalico(e.target.checked)} style={{ accentColor: '#06b6d4' }} />
                Itálico
              </label>

              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#cbd5e1', cursor: 'pointer' }}>
                <input type="checkbox" checked={moldura} onChange={(e) => setMoldura(e.target.checked)} style={{ accentColor: '#06b6d4' }} />
                Moldura
              </label>

              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#cbd5e1', cursor: 'pointer', gridColumn: 'span 2' }}>
                <input type="checkbox" checked={destaque} onChange={(e) => { const val = e.target.checked; setDestaque(val); if (val) setQuebraLinha(true); }} style={{ accentColor: '#06b6d4' }} />
                Destaque (Citação &gt;)
              </label>
            </div>
            ...
            <div className="form-actions" style={{ display: 'flex', gap: '6px' }}>
              ...
              {editingId && (
                <button type="button" className="btn-form-action btn-form-cancel" style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)' }} onClick={() => handleDelete(editingId)} title="Excluir Atendente">
                  <Trash2 size={12} />
                </button>
              )}

              <button type="button" className="btn-form-action btn-form-cancel" onClick={() => { setIsEditing(false); setEditingId(null); }}>
                <X size={12} />
              </button>
            </div>
          </form>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button type="button" className="btn-add-card" onClick={handleAddNewClick} style={{ padding: '4px 10px', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '3px' }}>
                <Plus size={11} /> Novo
              </button>
            </div>
                ...
                attendants.map((att, idx) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '16px 8px', background: att.isFavorite ? 'rgba(6, 182, 212, 0.05)' : 'rgba(255,255,255,0.01)',
                      border: att.isFavorite ? '1px solid rgba(6, 182, 212, 0.25)' : '1px solid rgba(255,255,255,0.04)',
                      borderRadius: '12px', position: 'relative', textAlign: 'center', minHeight: '100px', transition: 'all 0.15s ease'
                    }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(6, 182, 212, 0.4)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = att.isFavorite ? 'rgba(6, 182, 212, 0.25)' : 'rgba(255,255,255,0.04)'; e.currentTarget.style.transform = 'none'; }}
                  >
                    <button type="button" onClick={(e) => { e.stopPropagation(); handleToggleFavorite(att.id); }} style={{ position: 'absolute', top: '8px', right: '8px', background: 'transparent', border: 'none', cursor: 'pointer', color: att.isFavorite ? '#eab308' : 'rgba(255,255,255,0.15)', outline: 'none', padding: '2px' }} title={att.isFavorite ? 'Atendente Ativo (Principal)' : 'Definir como Atendente Ativo'}>
                    ...
                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: att.isFavorite ? 'rgba(6, 182, 212, 0.15)' : 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: att.isFavorite ? '#06b6d4' : '#94a3b8', marginBottom: '8px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 700 }}>{att.name.slice(0, 2).toUpperCase()}</span>
                    </div>
